class Solution {
    void dfsCountAndUpdate(int node, int prevNode, vector<vector<int>>& adjVec,
        ...
        int prevCount = freqMap[labels[node] - 'a'];	// STORE THE COUNT FROM PREVIOUS PART OF THE TREE
        ...
        solution[node] = freqMap[labels[node] - 'a'] - prevCount;		// REMOVE IT FROM THE TOTAL COUNT
    }
